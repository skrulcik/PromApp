...
/*
class PromDetailController:UITableViewController{
    //MARK: Local Variables
    private let INFO_SECT = 0
    private let DRESS_SECT = 1
    private var dressList:Array<SKDress> = []
    var prom:SKProm{
        didSet {
            updateDressData()
        }
    ...
    required init(coder aDecoder: NSCoder) {
        prom = SKProm.defaultProm()
        super.init(coder: aDecoder)
    }
    
    
    /* Updates dresses for prom, and refreshes view
    *   -Queries server for dresses from prom (if it exists)
    *   -Forces tableView to update cells
    */
    func updateDressData(){
        //Retrieve dress from server
        //Force table reload
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //Register cells
        tableView.registerNib(UINib(nibName: "DressCell", bundle: nil), forCellReuseIdentifier: "DressCell");
        tableView.registerNib(UINib(nibName: "PromCell", bundle: nil), forCellReuseIdentifier: "PromCell");
        
        //TODO: If user has editing ability, show an edit button in the top left
    }
    
    //MARK: UITableViewDataSource
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        println("indexPath \(indexPath)")
        /*if indexPath.section == INFO_SECT{
            // Only fill in data if prom has been assigned
            println("This is prom: \(prom.readableInfo())")
            if let cell:SKPromTableViewCell = tableView.dequeueReusableCellWithIdentifier("PromCell") as? SKPromTableViewCell{
                //Fill in cell with prom data
                cell.school.text = prom.schoolName
                cell.theme.text = prom.theme
                cell.date.text = prom.time
                cell.locationDescription.text = prom.locationDescription
                cell.address.text = prom.address
                //Tell cell's mapview to zoom in on a certain location
                var latitude:Double? = prom.preciseLocation.latitude
                var longitude:Double? = prom.preciseLocation.longitude
                if latitude != nil && longitude != nil {
                    let center = CLLocationCoordinate2D(latitude: latitude!, longitude: longitude!)
                    let region = MKCoordinateRegionMakeWithDistance(center, 250, 250)
                    cell.map.setRegion(region, animated: false)
                    //Show a pin at the address of the prom
        ...
        } else if indexPath.section == DRESS_SECT {
            if let cell:SKDressInfoTableViewCell = tableView.dequeueReusableCellWithIdentifier("DressCell") as? SKDressInfoTableViewCell{
                //Fill in Cell with Dress Data
                return cell
            }
        }*/
        return UITableViewCell()
    ...
    override func numberOfSectionsInTableView(tableView: UITableView) -> Int {
        return 2; //One for header, one for dresses
    }
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if section == INFO_SECT {
            return 1
        } else if section == DRESS_SECT {
